...
@register_op
class Multi_Decode(BaseOperator):
    def __init__(self):
        """ Transform the muiti_image data to numpy format following the rgb format
        """
        super(Multi_Decode, self).__init__()

    def apply(self, sample, context=None):
        """ load image if 'im_file' field is not empty but 'image' is"""
        if 'image' not in sample:
            with open(sample['vis_im_file'], 'rb') as f:
                sample['vis_image'] = f.read()
            sample.pop('vis_im_file')
            with open(sample['ir_im_file'], 'rb') as f:
                sample['ir_image'] = f.read()
            sample.pop('ir_im_file')

        try:
            vis_im = sample['vis_image']
            vis_data = np.frombuffer(vis_im, dtype='uint8')
            vis_im = cv2.imdecode(vis_data, 1)  # BGR mode, but need RGB mode
            if 'keep_ori_im' in sample and sample['keep_ori_im']:
                sample['vis_ori_image'] = vis_im
            vis_im = cv2.cvtColor(vis_im, cv2.COLOR_BGR2RGB)

            ir_im = sample['ir_image']
            ir_data = np.frombuffer(ir_im, dtype='uint8')
            ir_im = cv2.imdecode(ir_data, 1)  # BGR mode, but need RGB mode
            if 'keep_ori_im' in sample and sample['keep_ori_im']:
                sample['ir_ori_image'] = ir_im
            ir_im = cv2.cvtColor(ir_im, cv2.COLOR_BGR2RGB)
        except:
            vis_im = sample['vis_image']
            ir_im = sample['ir_image']

        sample['vis_image'] = vis_im
        sample['ir_image'] = ir_im
        if 'h' not in sample:
            sample['h'] = vis_im.shape[0]
        elif sample['h'] != vis_im.shape[0]:
            logger.warning(
                "The actual image height: {} is not equal to the "
                "height: {} in annotation, and update sample['h'] by actual "
                "image height.".format(vis_im.shape[0], sample['h']))
            sample['h'] = vis_im.shape[0]
        if 'w' not in sample:
            sample['w'] = vis_im.shape[1]
        elif sample['w'] != vis_im.shape[1]:
            logger.warning(
                "The actual image width: {} is not equal to the "
                "width: {} in annotation, and update sample['w'] by actual "
                "image width.".format(vis_im.shape[1], sample['w']))
            sample['w'] = vis_im.shape[1]

        sample['im_shape'] = np.array(vis_im.shape[:2], dtype=np.float32)
...
@register_op
class Multi_Permute(BaseOperator):
    def __init__(self):
        """
        Change the channel to be (C, H, W)
        """
        super(Multi_Permute, self).__init__()

    def apply(self, sample, context=None):
        vis_im = sample['vis_image']
        vis_im = vis_im.transpose((2, 0, 1))
        sample['vis_image'] = vis_im

        ir_im = sample['ir_image']
        ir_im = ir_im.transpose((2, 0, 1))
        sample['ir_image'] = ir_im
...
@register_op
class Multi_NormalizeImage(BaseOperator):
    def __init__(self,
                 mean=[0.485, 0.456, 0.406],
                 std=[0.229, 0.224, 0.225],
                 is_scale=True,
        ...
        Args:
            ...
            norm_type (str): type in ['mean_std', 'none']
        """
        super(Multi_NormalizeImage, self).__init__()
        self.mean = mean
        self.std = std
        self.is_scale = is_scale
        self.norm_type = norm_type
    ...
    def apply(self, sample, context=None):
        ...
        vis_im = vis_im.astype(np.float32, copy=False)
        ir_im = ir_im.astype(np.float32, copy=False)
        if self.is_scale:
            scale = 1.0 / 255.0
            vis_im *= scale
            ir_im *= scale
...
@register_op
class Multi_RandomDistort(BaseOperator):
    ...
    def __init__(self,
                 ...
                 prob=1.0):
        super(Multi_RandomDistort, self).__init__()
        self.hue = hue
    ...
    def apply(self, sample, context=None):
        if random.random() > self.prob:
            return sample
        vis_img = sample['vis_image']
        ir_img = sample['ir_image']
        if self.random_apply:
            ...
            for func in distortions:
                vis_img = func(vis_img)
                ir_img = func(ir_img)
            sample['vis_image'] = vis_img
            sample['ir_image'] = ir_img
            return sample

        vis_img = self.apply_brightness(vis_img)
        ir_img = self.apply_brightness(ir_img)
        mode = np.random.randint(0, 2)

        if mode:
            vis_img = self.apply_contrast(vis_img)
            ir_img = self.apply_contrast(ir_img)

        vis_img = self.apply_saturation(vis_img)
        ir_img = self.apply_saturation(ir_img)
        vis_img = self.apply_hue(vis_img)
        ir_img = self.apply_hue(ir_img)
        ...
        if self.random_channel:
            if np.random.randint(0, 2):
                vis_img = vis_img[..., np.random.permutation(3)]
                ir_img = ir_img[..., np.random.permutation(3)]
        sample['vis_image'] = vis_img
        sample['ir_image'] = ir_img
        return sample
...
@register_op
class Multi_RandomFlip(BaseOperator):
    def __init__(self, prob=0.5):
        """
        Args:
            prob (float): the probability of flipping image
        """
        super(Multi_RandomFlip, self).__init__()
        self.prob = prob
        if not (isinstance(self.prob, float)):
            raise TypeError("{}: input type is invalid.".format(self))
    ...
    def apply(self, sample, context=None):
        ...
        if np.random.uniform(0, 1) < self.prob:
            vis_im = sample['vis_image']
            ir_im = sample['ir_image']
            height, width = vis_im.shape[:2]
            vis_im = self.apply_image(vis_im)
            ir_im = self.apply_image(ir_im)
            if 'gt_bbox' in sample and len(sample['gt_bbox']) > 0:
                sample['gt_bbox'] = self.apply_bbox(sample['gt_bbox'], width)
            if 'gt_poly' in sample and len(sample['gt_poly']) > 0:
                sample['gt_poly'] = self.apply_segm(sample['gt_poly'], height,
...
@register_op
class Multi_Resize(BaseOperator):
    def __init__(self, target_size, keep_ratio, interp=cv2.INTER_LINEAR):
        """
        Resize image to target size. if keep_ratio is True,
        resize the image's long side to the maximum of target_size
        ...
        Args:
            ...
            interp (int): the interpolation method
        """
        super(Multi_Resize, self).__init__()
        self.keep_ratio = keep_ratio
        self.interp = interp
        if not isinstance(target_size, (Integral, Sequence)):
            raise TypeError(
    ...
    def apply(self, sample, context=None):
        ...
        if not isinstance(vis_im, np.ndarray):
            raise TypeError("{}: vis_image type is not numpy.".format(self))
        if not isinstance(ir_im, np.ndarray):
            raise TypeError("{}: ir_image type is not numpy.".format(self))

        # apply image
        if len(vis_im.shape) == 3:
            im_shape = vis_im.shape
        else:
            im_shape = vis_im[0].shape
        ...
        if len(vis_im.shape) == 3:
            ...
            resized_vis_images = []
            resized_ir_images = []
            for one_vis_im,one_ir_im in zip(vis_im,ir_im):
                applied_vis_im = self.apply_image(one_vis_im, [im_scale_x, im_scale_y])
                applied_ir_im = self.apply_image(one_ir_im, [im_scale_x, im_scale_y])
                resized_vis_images.append(applied_vis_im)
                resized_ir_images.append(applied_ir_im)

            sample['vis_image'] = np.array(resized_vis_images)
            sample['ir_image'] = np.array(resized_ir_images)
...
@register_op
class Multi_RandomExpand(BaseOperator):
    """Random expand the canvas.
    Args:
        ratio (float): maximum expansion ratio.
        prob (float): probability to expand.
    ...
    def __init__(self, ratio=4., prob=0.5, fill_value=(127.5, 127.5, 127.5)):
        super(Multi_RandomExpand, self).__init__()
        assert ratio > 1.01, "expand ratio must be larger than 1.01"
        self.ratio = ratio
        self.prob = prob
        assert isinstance(fill_value, (Number, Sequence)), \
...
@register_op
class Multi_RandomExpand_IRVIS(BaseOperator):
    """Random expand the canvas.
    Args:
        ratio (float): maximum expansion ratio.
        prob (float): probability to expand.
    ...
    def __init__(self, ratio=4., prob=0.5, fill_value=(127.5, 127.5, 127.5), fill_value_ir=(114.495, 114.495, 114.495)):
        super(Multi_RandomExpand_IRVIS, self).__init__()
        assert ratio > 1.01, "expand ratio must be larger than 1.01"
        self.ratio = ratio
        self.prob = prob
        assert isinstance(fill_value, (Number, Sequence)), \
        ...
        assert isinstance(fill_value_ir, (Number, Sequence)), \
            "fill value ir must be either float or sequence"
        if isinstance(fill_value_ir, Number):
            fill_value_ir = (fill_value_ir,) * 3
        if not isinstance(fill_value_ir, tuple):
            fill_value_ir = tuple(fill_value_ir)
        self.fill_value_ir = fill_value_ir
    ...
    def apply(self, sample, context=None):
        ...
        pad = Multi_Pad_IRVIS(size,
                  pad_mode=-1,
                  offsets=offsets,
                  fill_value=self.fill_value,
                  fill_value_ir=self.fill_value_ir)
...
@register_op
class Multi_RandomCrop(BaseOperator):
    """Random crop image and bboxes.
    Args:
        aspect_ratio (list): aspect ratio of cropped region.
            in [min, max] format.
    ...
    def __init__(self,
                 ...
                 ioumode="iou",
                 prob=1.0):
        super(Multi_RandomCrop, self).__init__()
        self.aspect_ratio = aspect_ratio
        self.thresholds = thresholds
        self.scaling = scaling
        self.num_attempts = num_attempts
    ...
    def random_crop(self, sample, fake_bboxes=False):
        ...
        for thresh in thresholds:
            ...
            if found:
                ...
                sample['vis_image'] = self._crop_image(sample['vis_image'], crop_box)
                sample['ir_image'] = self._crop_image(sample['ir_image'], crop_box)
                if fake_bboxes == True:
                    return sample
...
@register_op
class Multi_Mixup(BaseOperator):
    def __init__(self, alpha=1.5, beta=1.5):
        """ Mixup image and gt_bbbox/gt_score
        Args:
            alpha (float): alpha parameter of beta distribute
            beta (float): beta parameter of beta distribute
        """
        super(Multi_Mixup, self).__init__()
        self.alpha = alpha
        self.beta = beta
        if self.alpha <= 0.0:
            raise ValueError("alpha shold be positive in {}".format(self))
    ...
    def __call__(self, sample, context=None):
        ...
        if factor <= 0.0:
            return sample[1]
        vis_im = self.apply_image(sample[0]['vis_image'], sample[1]['vis_image'], factor)
        ir_im = self.apply_image(sample[0]['ir_image'], sample[1]['ir_image'], factor)
        result = copy.deepcopy(sample[0])
        result['vis_image'] = vis_im
        result['ir_image'] = ir_im
        # apply bbox and score
        if 'gt_bbox' in sample[0]:
            gt_bbox1 = sample[0]['gt_bbox']
            gt_bbox2 = sample[1]['gt_bbox']
...
class Multi_Pad(BaseOperator):
    ...
    def apply(self, sample, context=None):
        ...
        sample['vis_image'] = self.apply_image(vis_im, offsets, im_size, size)
        sample['ir_image'] = self.apply_image(ir_im, offsets, im_size, size)
...
@register_op
class Multi_Pad_IRVIS(BaseOperator):
    def __init__(self,
                 size=None,
                 size_divisor=32,
                 pad_mode=0,
                 offsets=None,
                 fill_value=(127.5, 127.5, 127.5),
                 fill_value_ir=(114.495, 114.495, 114.495)):
        """
        Pad image to a specified size or multiple of size_divisor.
        Args:
            size (int, Sequence): image target size, if None, pad to multiple of size_divisor, default None
            ...
            fill_value (bool): rgb value of pad area, default (127.5, 127.5, 127.5)
        """
        super(Multi_Pad_IRVIS, self).__init__()
    ...
    def apply_image_ir(self, image, offsets, im_size, size):
        x, y = offsets
        im_h, im_w = im_size
        h, w = size
        canvas = np.ones((h, w, 3), dtype=np.float32)
        canvas *= np.array(self.fill_value_ir, dtype=np.float32)
        canvas[y:y + im_h, x:x + im_w, :] = image.astype(np.float32)
        return canvas
    ...
    def apply(self, sample, context=None):
        ...
        sample['vis_image'] = self.apply_image(vis_im, offsets, im_size, size)
        sample['ir_image'] = self.apply_image_ir(ir_im, offsets, im_size, size)
